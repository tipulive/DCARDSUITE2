...
<head>
    ...
    <style>
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        /* Search & reset styles */
        .search-with-reset { position: relative; }
        .search-with-reset input { padding-right: 40px; }
        .search-reset-icon {
            position: absolute;
            right: 10px;
            top: 50%;
        ...
        .search-reset-icon:hover { background: #e9ecef; color: #dc3545; }
        .not-found-message {
            text-align: center;
            padding: 24px 16px;
            color: #6c757d;
        ...
        .not-found-message i { font-size: 2rem; display: block; margin-bottom: 8px; opacity: 0.5; }
    </style>
</head>
<body>
...
<!-- Product Picker Modal (No "Create New" - only existing products) -->
...
<script>
    ...
    try {
        ...
        console.log('API response sample:', data); // Debug: see actual structure

        // ✅ FIX 2: Adapt to your real API structure
        // Assuming your API returns an array of products with fields: id, productCode, productName, price, etc.
        // Adjust the mapping according to your backend response
        let products = [];
        ...
        // Map to the format expected by the picker (id, name, category, price)
        ...
        updateAPIStatus('offline'); // Optional: show offline indicator
        return [...FALLBACK_PRODUCTS];
    }
}
    function updateAPIStatus(status) {
        $('#apiStatus').html(status === 'online' ? '<i class="bi bi-cloud-check-fill text-success"></i> Server connected' : '<i class="bi bi-cloud-slash-fill text-warning"></i> Offline mode');
    }
    ...
    function updateProductUIs() {
        ...
        else currentAllowedArray.forEach(prod => {
            allowedRibbon.append(`<span class="product-badge allowed-badge"><span>${escapeHtml(prod)}</span><i class="bi bi-x-circle-fill remove-allowed" data-product="${escapeHtml(prod)}"></i></span>`);
        });
        ...
        else currentExcludedArray.forEach(prod => {
            excludedRibbon.append(`<span class="product-badge excluded-badge"><span>${escapeHtml(prod)}</span><i class="bi bi-x-circle-fill remove-excluded" data-product="${escapeHtml(prod)}"></i></span>`);
        });
    ...
    $("#confirmPickerSelection").on('click', function() {
        if (pickerMode === 'allowed') {
            ...
            selectedTempProducts.forEach(p => {
                if (!currentInStockItems.some(i => i.productName === p)) currentInStockItems.push({ productName: p, qty: 1 });
            });
        }
        updateProductUIs();
        ...
        $.ajax({
            ...
            { promoArrId: myIndex,
                ...
                /* products: [...currentAllowedArray],
                 ...
                 mainPromoId: mainPromoId, app_vers: '{{env('APP_VERS')}}' },
            success: function (data) { renderPromotions(); console.log("SUCCESS:", data); },
            ...
            success: function(data) {
                if (data.status) {
                    ...
                    if (!promotionsData.length) {
                        grid.html(`<div class="col-12"><div class="empty-state"><i class="bi bi-ticket-perforated fs-1 text-muted"></i><h4>No promotions</h4></div></div>`);
                        return;
                    }
                    let html = '';
    ...
    $(document).ready(async function(){
        ...
        $('#promoForm').on('submit', (e) => { e.preventDefault(); savePromotion(); });
